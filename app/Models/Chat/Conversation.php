 ...
 * @property-read \Illuminate\Database\Eloquent\Collection|\App\Models\Chat\Participation[] $participants
 * @property-read int|null $participants_count
 * @property-read \App\Models\Admin\Support\Ticket|null $ticket
 ...
 * @property-read \App\Models\Products\Advertise|null $advertise
 ...
 * @property-read \Illuminate\Database\Eloquent\Collection|\App\Models\Chat\MessageNotification[] $unread_messages
 ...
 * @method static \Illuminate\Database\Eloquent\Builder|Conversation isActive()
 * @mixin \Eloquent
 */
class Conversation extends BaseModel
{
    ...
    public static array $selectedRelationsConversation = [
        'ticket:id,status',
        'advertise.fake_user_avatar',
        'advertise.gallery',
        'advertise.category',
        'advertise.city',
        'participants:conversation_id,messageable_id,messageable_type',
        'participants.messageable:id,first_name,last_name,created_at',
     ...
     * @return mixed
     */
    public function participantReceiver(Model $sender): mixed {
        return $this->participants()
            ->where([
                'conversation_id'  => $this->getKey(),
                'messageable_type' => $sender->getMorphClass(),
            ])
            ->where('messageable_id', '<>', $sender->getKey())
            ->first();
    }
...
//        }
    ...
    public function scopeHasFilter($query, array $data): mixed {
        ...
        return $query
            ...
            ->when(($conType === true || $conType === ChatTypesEnum::RESELL), fn($q) => $q->whereStarted(true))
            ->when($search, fn($q) => $q->isSearch($participant, $search))
            ->when(is_string($filters['conversation_type']), function ($q) use ($participant, $conType, $search, $filters) {
                $q
                    ->isChat($conType)
                    ->isSupport($conType, $filters)
     ...
     * @return mixed
     */
    public function scopeIsSupport($query, string $conType, array $filters = null): mixed {
        $isSupport = $conType === ChatTypesEnum::SUPPORT;
        return $query->when($isSupport, function ($when) use ($filters) {
            $when
                ->when(isset($filters['theme_id']) && $filters['theme_id'], function ($q) use ($filters) {
    ...
    public function scopeIsBuying($query, Model $model, string $conType): mixed {
        $isBuying = $conType === ChatTypesEnum::BUYING;
    ...
    public function scopeIsResell($query, Model $model, string $conType): mixed {
        $isResell = $conType === ChatTypesEnum::RESELL;
     ...
     * @return mixed
     */
    public function scopeIsChat($query, string $conType): mixed {
        $isChat = $conType !== ChatTypesEnum::SUPPORT;

        return $query->when($isChat, function ($q) {
    ...
    public function scopeIsSearch($query, Model $model, string $search = null): mixed {
        return $query->when($search, function ($q) use ($model, $search) {
            ...
            $q
                ->where(function ($q) use ($model, $search) {
                    $q
                        ->whereHas('advertise', function ($q) use ($search) {
                            $q->where('name', 'LIKE', "%$search%");
                        })
                        ->orWhereHas('participants.messageable', function ($q) use ($search, $model) {
                            $q
                                ->where(function ($q) use ($search) {
                                    $q->where('first_name', 'LIKE', "%$search%")->orWhere('last_name', 'LIKE', "%$search%");
                                })
                                ->where('messageable_id', '<>', $model->getKey());
                        });
                });
        });
    }
