        ...
        if ($search && is_array($fieldsSearchable) && count($fieldsSearchable)) {
            ...
            $model = $model->when(isset($searchData['search_text']), function ($query) use ($searchData) {
                ...
                $query->where(function ($q) use ($text) {
                    ...
                    $q->where('advertises.name', 'LIKE', "%$text%");
                });
            });
            ...
            $model = $model->when((isset($searchData['city_id']) && (isset($searchData['mil']) || isset($searchData['km']))),
                function ($query) use (&$searchData) {
                    ...
                    if($city) {
                        unset($searchData['city_id']);
                        $milOrKm = isset($searchData['mil']) ? self::EARTH_RADIUS_IN_MILES : self::EARTH_RADIUS_IN_KILOMETERS;
                        $query
                            ->selectRaw("advertises.*,ct.name as city_name,
                             ($milOrKm * acos (
                                  cos (radians($city->latitude))
                                  * cos(radians(advertises.latitude))
                                  * cos(radians(advertises.longitude) - radians($city->longitude))
                                  + sin(radians($city->latitude))
                                  * sin(radians(advertises.latitude))
                                )
                             ) AS distance")
