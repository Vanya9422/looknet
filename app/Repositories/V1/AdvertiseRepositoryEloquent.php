...
class AdvertiseRepositoryEloquent extends BaseRepository implements AdvertiseRepository {
    ...
    protected $fieldSearchable = [
        ...
        'city.country_id',
        'answers.filter_id',
        'answers.number_value',
        'answers.string_value',
        'city.state.name' => 'LIKE',
    ...
    public function searchTexts(string $search = null): LengthAwarePaginator {
        ...
        if ($search) {
            $query = $table
                ...
                ->leftJoin('advertises', function($join) {
                    $join->on('advertises.name', 'LIKE', DB::raw("CONCAT('%',us.search,'%')"));
                    $join->whereIn('advertises.status', AdvertiseStatus::getValues(['Active', 'NotVerified']));
                    $join->whereNull('advertises.deleted_at');
                })
            ...
            if (isset($attributes['answers'])) {
                ...
                foreach ($attributes['answers'] as $answer) {
                    ...
                    if (isset($answer['filter_id'])) {
                        $newAnswer = FilterAnswer::create($answer);
                ...
                $attributes['answer_ids'] = $answer_ids;
            }

            $advertise = parent::create($attributes);

            if (isset($answer_ids)) {
                $advertise->answers()->sync($answer_ids);
            }
    ...
    public function updateAdvertise(array $attributes, Advertise $advertise): Advertise {
        DB::transaction(function () use ($attributes, &$advertise) {
                ...
                foreach ($attributes['answers'] as $answer) {
                    ...
                    if (isset($answer['filter_id'])) {
                        ...
                        if (count($answer) === 1) {
                            ...
                            $answer_id = $answerRepo->update($answer, $answer['id'])->id;
                        }
                    }
            ...
            if (isset($answer_ids)) $advertise->answers()->sync($answer_ids);
    ...
    public function changeStatus(AdvertisesRequest $request, Advertise $advertise, int $moderator_id = null): void {
        ...
        $data = ['description' => $request->get('refusal_comment')];

        if ($refusal_id = $request->get('refusal_id')) $data['refusal_id'] = $refusal_id;

        if ($data['description'] != null || isset($data['refusal_id'])) $advertise->complaint()->create($data);

        $advertise->update(['status' => $status]);
     ...
     * @return mixed
     */
    public function getTopAdvertisesRandomOrder() {
        $select = Advertise::$selectedFields;
        $select[] = 'cty.name as city_name';

        return $this->getModel()->newQuery()
            ->join('subscriptions', function ($join) {
                $join->on('subscriptions.owner_id', '=', 'advertises.id');
                $join->where('subscriptions.owner_type', '=', Advertise::class);
            })
            ->leftJoin('cities as cty', 'advertises.city_id', '=', 'cty.id')
            ->where('subscriptions.expired_top_days', '>=', \Carbon\Carbon::now())
            ->where('subscriptions.status', true) // Предполагая, что статус должен быть true
            ->where('advertises.published', true)
            ->inRandomOrder()
            ->limit(12)
            ->get($select);
    }
    ...
    private function addPicturesIfExistsFile(Advertise $advertise, array $attributes, bool $withDelete = false): void {
        ...
        if (isset($attributes['file'])) {
            ...
            $advertise->clearMediaCollection(\App\Enums\MediaCollections::USER_FAKE_AVATAR_ADVERTISE);

            /**
             * Добовляем Новую
             */
            $this->addResponsiveImage($advertise, $attributes['file'], \App\Enums\MediaCollections::USER_FAKE_AVATAR_ADVERTISE);
        }
    }
}
