...
class AuthController extends Controller {
        ...
        try {
            ...
            if ($user->isBanned()) {
                return response()->json([
                    'status' => false,
                    'blocked' => true,
                    'message' => __('messages.BLOCKED_USER')
            ...
            if (!\Auth::attempt($request->only([$field, 'password']))) {
                return response()->json([
                    'status' => false,
                    'message' => __('messages.CAN_NOT_LOGIN')
                ], Response::HTTP_UNAUTHORIZED);
            }
     ...
     * @param UserSettingsCodeConfirmationHandlerInterface $userConfirmationHandler
     * @return JsonResponse
     */
    public function sendCodeAgain(
        SendCodeAgainRequest $request,
        UserSettingsCodeConfirmationHandlerInterface $userConfirmationHandler
    ): JsonResponse {
        try {
            $typeConfirm = $request->get('confirmation_type');
            $user = $this->userRepository->getUserByEmailOrPhone($request->all(), false);

            $userConfirmationHandler->handle($user, $request->all());
